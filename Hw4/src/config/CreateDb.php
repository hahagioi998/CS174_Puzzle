<?php
namespace zihaolinqianhuifan\hw3;
...
$con=new \mysqli($hostname, Config::user, Config::password);
$result=$con->query("SHOW DATABASES WHERE `Database`=\"hw3\";")->fetch_all();
if(empty($result)){
    $con->query("create database hw3;");
    $con->query($sql);
    mysqli_multi_query($con, $sql);
    echo "database initialized";
}
else{
  echo "no need to initialized";
}